...
void handleMqttHost() {
  ...
  String response = (success) ? "{\"status\": \"ok\"}" : "{\"status\": \"error\", \"error\":\"Failed to set " + server.arg("mqtt_host") + " as mqtt host.\"}";
...
void beginST() {
  ...
  //Serial.println(eepromHandler.getSsid());
  //Serial.println(eepromHandler.getPassword());
...
void setup(void) {
 ...
 if (button_pressed_on_boot) {
  ...
  //Serial.println("before mqtt init");
  mqtt.init();
